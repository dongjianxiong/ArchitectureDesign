//  代码地址: https://github.com/iphone5solo/PYPhotosView
//  代码地址: http://code4app.com/thread-8612-1-1.html
//  Created by CoderKo1o.
//  Copyright © 2016年 iphone5solo. All rights reserved.
//
#import "PYPhotoBrowserConst.h"
const CGFloat PYPhotoMargin = 5;   // 图片之间的默认间距
const CGFloat PYPhotoWidth = 70;    // 图片的默认宽度
const CGFloat PYPhotoHeight = 70;   // 图片的默认高度
const CGFloat PYPhotosMaxCol = 3;  // 图片每行默认最多个数
const CGFloat PYPreviewPhotoSpacing = 30;   // 预览图片时，图片的间距
const CGFloat PYPreviewPhotoMaxScale = 2;  // 预览图片时，图片最大放大倍数
const CGFloat PYImagesMaxCountWhenWillCompose = 9; // 在发布状态时，最多可以上传的图片张数

NSString *const PYPhotoBrowseViewKey = @"PYPhotoBrowseViewKey";

NSString *const PYBigImageDidClickedNotification = @"PYBigImageDidClickedNotification";

NSString *const PYSmallgImageDidClickedNotification = @"PYSmallImageDidClickedNotification";

NSString *const PYImagePageDidChangedNotification = @"PYImagePageDidChangedNotification";

NSString *const PYPreviewImagesDidChangedNotification = @"PYPreviewImagesDidChangedNotification";

NSString *const PYChangeNavgationBarStateNotification = @"PYChangeNavgationBarStateNotification";

NSString *const PYAddImageDidClickedNotification = @"PYAddImageDidClickedNotification";

NSString *const PYCollectionViewDidScrollNotification = @"NSString *const PYCollectionViewDidScrollNotification";

